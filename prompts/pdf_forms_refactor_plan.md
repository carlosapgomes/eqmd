...
class MedicalProcedure(models.Model):
    ...
    procedure_type = models.CharField(max_length=10, choices=[
        ('APAC', 'Ambulatorial'),
        ('AIH', 'Hospitalar'),
        ('BOTH', 'Ambos')
    ])
...
- Type classification (APAC/AIH/Both)
...
uv run python manage.py import_procedures --file procedures.csv --type APAC
```
...
GET /api/procedures/search/?q=diabetes&type=APAC&limit=10
```
...
This refactor maintains the simplicity and speed of the current system while adding the flexibility needed for complex national forms. The phased approach ensures zero downtime and provides immediate value with each phase.
