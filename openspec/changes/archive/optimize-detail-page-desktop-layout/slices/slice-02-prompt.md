...
4. `openspec/changes/optimize-detail-page-desktop-layout/slices/slice-02.md`

## Rules

- Follow TDD strictly (RED -> GREEN -> CLEAN -> VERIFY).
- Stay inside the listed files and scope.
- Keep test changes focused in `apps/simplenotes/tests/test_detail_context_ui.py`.
- Replace only the desktop sidebar layout in `simplenote_detail`.
...
- Render the simple note metadata card as a full-width desktop top card below it.
...
- Keep simple note actions and event datetime visible.
...
- Do not touch `dailynote_detail` or `historyandphysical_detail` in this slice.
