...
4. `openspec/changes/optimize-detail-page-desktop-layout/slices/slice-01.md`

## Rules

- Follow TDD strictly (RED -> GREEN -> CLEAN -> VERIFY).
- Stay inside the listed files and scope.
- Keep test changes focused in `apps/dailynotes/tests/test_detail_context_ui.py`.
- Replace only the desktop sidebar layout in `dailynote_detail`.
...
- Render the daily note metadata card as a full-width desktop top card below it.
...
- Keep daily note actions and event datetime visible.
...
- Do not touch `simplenote_detail` or `historyandphysical_detail` in this slice.
